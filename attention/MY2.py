...
class SpatialAttention(layers.Layer):
    def __init__(self, kernels, stride=2):
        ...
        self.spacital = Sequential([
            regularized_padded_conv(kernels, (7, 7), strides=1),
            layers.BatchNormalization(),
            layers.ReLU()
        ])
    ...
    def call(self, inputs, training=False):
       ...
       # x = self.features(inputs, training=training)
        ...
        up1 = tf.image.resize(down1, size=(tf.shape(inputs)[1], tf.shape(inputs)[2]))

        down2 = self.down2(inputs, training=training)
        # if down.shape != x1.shape:

        up2 = tf.image.resize(down2, size=(tf.shape(inputs)[1], tf.shape(inputs)[2]), method="nearest")
